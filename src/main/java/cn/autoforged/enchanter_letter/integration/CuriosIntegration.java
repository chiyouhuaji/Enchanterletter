...
public class CuriosIntegration {
    ...
    private static final ResourceLocation ALL_LETTERS_PREDICATE =
            new ResourceLocation("enchanter_letter", "all_letters");
    private static boolean predicateRegistered = false;

    /**
     * 绑定拦截窗口：玩家 shift+右键手札执行绑定/解绑时置位，
     * 期间本模组物品对 Curios 槽位的有效性判定返回 false，
     * 阻止 Curios 的 curioRightClick 抢先把物品装备进饰品槽（不取消事件，物品 use() 照常触发绑定）。
     */
    private static volatile boolean bindInterceptActive = false;

    public static void beginBindIntercept() {
        bindInterceptActive = true;
    }

    public static void endBindIntercept() {
        bindInterceptActive = false;
    }

    public static boolean isBindInterceptActive() {
        return bindInterceptActive;
    }

    /** 绑定拦截窗口内是否应拒绝本模组物品（供槽位谓词调用）。 */
    private static boolean shouldRejectDuringBind(ItemStack stack) {
        return bindInterceptActive && isOurAccessoryItem(stack);
    }
        ...
        try {
            ...
            Object proxy = java.lang.reflect.Proxy.newProxyInstance(
                    ...
                    (obj, method, args) -> {
                        ...
                        // 关键：canEquipFromUse 必须返回 false！
                        // Curios 1.20.1 的 curioRightClick 在 isItemValid && canEquipFromUse 时
                        // 会拦截右键并自动装备，导致手札的 shift+右键绑定（长按 2 秒）永远无法触发。
                        // 手札仍可通过饰品界面/槽位拖放装备（canEquip=true）。
                        if ("canEquipFromUse".equals(name)) return false;
                        // 1.20.1 及旧版 Curios：getSlots 决定物品可放入哪些槽位，返回全部槽位 id
                        if ("getSlots".equals(name)) return getAllSlotIds();
            ...
            if (!predicateRegistered) {
                registerPredicateMethod.invoke(null, ALL_LETTERS_PREDICATE, (Predicate<Object>) result -> {
                    try {
                        ...
                        // 绑定拦截窗口内拒绝，阻止 Curios 右键抢先装备
                        if (shouldRejectDuringBind(stack)) return false;
    ...
    private static void wrapTagPredicate() {
        ...
        try {
            ...
            ResourceLocation tagId = new ResourceLocation("curios", "tag");
    ...
    private static List<String> cachedAllSlotIds;

    /** 解析 Curios 全部槽位 id（旧版 Curios 的 ICurioItem.getSlots 需要返回可放入槽位列表）。 */
    private static synchronized List<String> getAllSlotIds() {
        if (cachedAllSlotIds != null) return cachedAllSlotIds;
        List<String> result = new ArrayList<>();
        try {
            if (getSlotHelperMethod != null && getSlotTypeIdsMethod != null) {
                Object helper = getSlotHelperMethod.invoke(null);
                Object ids = getSlotTypeIdsMethod.invoke(helper);
                if (ids instanceof Set<?> set) {
                    for (Object id : set) result.add(String.valueOf(id));
                }
            }
        } catch (Exception ignored) {
        }
        cachedAllSlotIds = result;
        return result;
    }
    ...
    /**
     * 兼容 Optional 与 LazyOptional（Forge 1.20.1 的 CuriosApi.getCuriosInventory 返回
     * LazyOptional<ICuriosItemHandler>；NeoForge 1.21.1 返回 Optional）。返回内部 handler，无则 null。
     */
    private static Object resolveCuriosInventory(Object result) {
        ...
        try {
            ...
            if (!((Boolean) isPresent.invoke(result))) return null;
            Method resolve = result.getClass().getMethod("resolve");
            Object opt = resolve.invoke(result);
    ...
    public static List<ItemStack> getCuriosStacks(LivingEntity player) {
        if (!LOADED) return List.of();
        List<ItemStack> result = new ArrayList<>();
        try {
            // Forge 1.20.1 的 CuriosApi.getCuriosInventory 返回 LazyOptional（NeoForge 1.21.1 为 Optional），
            // 两种都要兼容，否则强转失败导致饰品栏扫描静默失效。
            Object handler = resolveCuriosInventory(getInventoryMethod.invoke(null, player));
    ...
    public static void ejectStacksWhere(Player player, Predicate<ItemStack> predicate, Consumer<ItemStack> onEjected) {
        if (!LOADED) return;
        try {
            // 兼容 LazyOptional（Forge 1.20.1）与 Optional（NeoForge 1.21.1）
            Object handler = resolveCuriosInventory(getInventoryMethod.invoke(null, player));
    ...
    public static boolean slotContains(Player player, String slotType, int slotIndex, ItemStack expected) {
        ...
        if (handler == null) return false;
        ItemStack current = getStackFromSlot(handler, slotType, slotIndex);
        return current != null && !current.isEmpty() && ItemStack.isSameItemSameTags(current, expected);
    }

    /** 解析玩家的 Curios handler（能力未就绪返回 null）。 */
    private static Object resolveCuriosHandler(Player player) {
        try {
            return resolveCuriosInventory(getInventoryMethod.invoke(null, player));
        } catch (Exception e) {
            return null;
        }
    }
