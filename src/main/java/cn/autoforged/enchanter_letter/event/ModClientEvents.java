...
@Mod.EventBusSubscriber(modid = UsefulMagicEnchanterLetterMod.MOD_ID, bus = Mod.EventBusSubscriber.Bus.FORGE, value = Dist.CLIENT)
public class ModClientEvents {
    private static final ResourceLocation HUD_OVERLAY =
            new ResourceLocation(UsefulMagicEnchanterLetterMod.MOD_ID, "magic_letter_hud");
    ...
    /**
     * /letterstorage 截取模式：在客户端 tick 最前（Phase.START）抢先消费“攻击”键击，
     * 拦截原版“空手左键攻击”，把准星指向的实体 id 上报服务端。
     * 仅在交互模式开启且主手为空时生效（此时原版 handleKeybinds 收不到该键击，不会发起攻击）。
     */
    @SubscribeEvent
